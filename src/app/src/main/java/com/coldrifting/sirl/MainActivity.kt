        ...
        NavHost(
            ...
            navigation<RouteCart>(startDestination = RouteCartSelect) {
                composable<RouteCartSelect> {
                    val selectedRecipes by viewModel.cart.selectedRecipes.collectAsState()
                    val remainingRecipes by viewModel.cart.availableRecipes.collectAsState()

                    val selectedItems by viewModel.cart.selectedItems.collectAsState()
                    val remainingItems by viewModel.cart.availableItems.collectAsState()
                    ...
                    CartSelect(
                        ...
                        cartItemsDone = shoppingCartList.flatMap { i -> i.items }.count{i -> i.checked},
                        cartItemsTotal = shoppingCartList.flatMap { i -> i.items }.size,
                        cartRecipes = selectedRecipes,
                        cartItems = selectedItems,
                        availableEntries = remainingRecipes,
                        availableItemEntries = remainingItems,
                        onAddRecipeToCart = viewModel.cart::addRecipeToCart,
                        onDeleteCartRecipe = viewModel.cart::removeRecipeFromCart,
                        updateRecipeInCart = viewModel.cart::updateRecipeInCart,
                        clearCartSelection = viewModel.cart::clearCartSelection,
                        onAddItemToCart = viewModel.cart::addItemToCart,
                        onDeleteCartItem = viewModel.cart::removeItemFromCart,
                        getDefaultItemType = viewModel.items::getDefaultItemType,
                        updateItemInCart = viewModel.cart::updateItemInCart
                    )
                }
